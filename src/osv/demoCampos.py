...
def goFaultOrientScan():
  ...
  fb = readImage3DL(fbfile) # computed planarity
  gx = copy(300,600,400,100,100,550,gx)
  fb = copy(300,600,400,100,100,550,fb)

  fos = FaultOrientScanner3(sigmaPhi,sigmaTheta)
  if not plotOnly:
    fe,fp,ft = fos.scan(minPhi,maxPhi,minTheta,maxTheta,sub(1,fb))
  ...
  print min(fe) 
  print max(fe) 
  plot3(gx,fb,cmin=0.25,cmax=1.0,cmap=jetRamp(1.0),
